...
namespace XUDPConnections
{
    public class XUdpListener : IDisposable
    {
        ...
        private readonly ConcurrentDictionary<IPEndPoint, Connection> connections;
        ...
        public XUdpListener(int port)
        {
            ...
            connections = new ConcurrentDictionary<IPEndPoint, Connection>();
        ...
        public async Task<Connection> AcceptConnectionAsync()
        {
        ...
        private async Task ReceiveLoopAsync()
        {
            while (!cts.IsCancellationRequested)
            {
                ...
                if (!connections.TryGetValue(result.RemoteEndPoint, out var conn))
                {
                    ...
                    {
                        conn = new Connection(udpClient, result.RemoteEndPoint);
        ...
        // Inner connection class (server-side handle)
        public class Connection
        {
            ...
            public Connection(UdpClient udpClient, IPEndPoint remoteEndPoint)
            {
